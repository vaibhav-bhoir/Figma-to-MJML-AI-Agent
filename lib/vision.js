...
/**
 * Analyze image using AI with fallback options
 * @param {Buffer|string} imageData - Image data or base64 string
 * @param {Object} options - Analysis options
 * @returns {Object} Image analysis with metadata
 */
export async function analyzeImageWithAI(imageData, options = {}) {
  try {
    // Try OpenAI first if available
    if (process.env.OPENAI_API_KEY) {
      return await analyzeImageWithOpenAI(imageData, options);
    }
    
    // Fall back to intelligent image analysis
    console.log('🔍 Using intelligent image analysis fallback...');
    return await analyzeImageIntelligently(imageData, options);
    
  } catch (error) {
    console.warn('⚠️ AI image analysis failed, using basic analysis:', error.message);
    return await analyzeImageIntelligently(imageData, options);
  }
}
 ...
 * @param {Buffer|string} imageData - Image data or base64 string
 * @param {Object} options - Analysis options
 * @returns {Object} Image analysis description
 */
async function analyzeImageWithOpenAI(imageData, options = {}) {
...
/**
 * Intelligent image analysis fallback (no AI required)
 * @param {Buffer} imageData - Image buffer
 * @param {Object} options - Analysis options
 * @returns {Object} Analysis result with intelligent guesses
 */
export async function analyzeImageIntelligently(imageData, options = {}) {
  const metadata = extractImageMetadata(imageData);
  const colors = await extractImageColors(imageData);
  
  // Calculate aspect ratio and make educated guesses about layout
  ...
  // Analyze dimensions to guess layout type
  if (aspectRatio > 2) {
    layoutType = 'banner';
    suggestedStructure = ['header-banner', 'content-section', 'footer'];
  } else if (aspectRatio > 1.5) {
    layoutType = 'landscape';
    suggestedStructure = ['header', 'hero-image', 'content-columns', 'cta-section'];
  } else if (aspectRatio < 0.7) {
    layoutType = 'portrait';
    suggestedStructure = ['header', 'content-stack', 'image-section', 'footer'];
  } else {
    layoutType = 'square';
    suggestedStructure = ['centered-content', 'image-feature', 'action-section'];
  }
  
  // Generate intelligent analysis text
  const analysis = `Image Analysis (Intelligent Fallback):
- Format: ${format.toUpperCase()} (${(size / 1024 / 1024).toFixed(1)}MB)
- Dimensions: ${width}×${height}px (${aspectRatio.toFixed(2)} ratio)
- Layout Type: ${layoutType}
- Suggested Email Structure: ${suggestedStructure.join(' → ')}
- Dominant Colors: ${colors.slice(0, 3).join(', ')}
- Email Suitability: ${width >= 320 && width <= 800 ? 'Excellent' : 'Needs resizing'}
- Mobile Friendly: ${aspectRatio <= 2 ? 'Yes' : 'May need adjustment'}

Recommended Template Approach:
Based on the ${layoutType} format, this image would work well as ${getTemplateRecommendation(layoutType, aspectRatio)}.`;
...
/**
 * Get template recommendation based on layout analysis
 */
function getTemplateRecommendation(layoutType, aspectRatio) {
  switch (layoutType) {
    case 'banner':
      return 'a header banner with content sections below';
    case 'landscape':
      return 'a hero image with side content or full-width sections';
    case 'portrait':
      return 'a stacked layout with the image as a featured element';
    case 'square':
      return 'a centered feature with balanced content around it';
    default:
      return 'a responsive email template adapted to the image proportions';
  }
}
